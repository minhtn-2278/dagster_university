...
def trips_by_week_answer(context: AssetExecutionContext, database: DuckDBResource) -> None:
    """
    ...
    try:
        ...
        existing = pd.read_csv('data/outputs/trips_by_week.csv')
        existing = existing[existing["period"] != period_to_fetch]
        existing = pd.concat([existing, aggregate]).sort_values(by="period")
        existing.to_csv('data/outputs/trips_by_week.csv', index=False)
    except FileNotFoundError:
        aggregate.to_csv('data/outputs/trips_by_week.csv', index=False)
