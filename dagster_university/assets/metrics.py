...
def trips_by_week_practice() -> None:
    query = f"""
        copy (
            select distinct
                case
                    when dayofweek(pickup_datetime) = 0 then strftime(pickup_datetime, '%Y-%m-%d')
                    else strftime(pickup_datetime + INTERVAL (7 - dayofweek(pickup_datetime)) DAY, '%Y-%m-%d')
                end as period,
...
def trips_by_week_answer() -> None:
    ...
    result.to_csv('data/outputs/trips_by_week2.csv', index=False)
